...
export async function generateMetadata({ params }: { params: { id: string } }) {
  ...
  const title = savings > 0 ? `I could save $${savings}/mo on AI tools — SpendLens` : 'My AI stack is optimised — SpendLens'
  return {
    title,
    description: 'Free AI spend audit for startups.',
    openGraph: { title, description: 'Free AI spend audit for startups.', siteName: 'SpendLens', type: 'website' },
    twitter: { card: 'summary_large_image', title, description: 'Free AI spend audit for startups.' },
  }
}
  ...
  return (
    <main className="min-h-screen bg-gray-50">
      <nav className="flex items-center justify-between px-8 py-5 border-b border-gray-100 bg-white">
        ...
        <a href="/audit" className="bg-emerald-600 text-white px-5 py-2 rounded-full text-sm font-medium hover:bg-emerald-700 transition">Audit My Stack</a>
      </nav>
      <div className="max-w-2xl mx-auto px-6 py-12">
        <p className="text-center text-gray-400 text-sm mb-6">Someone shared their AI spend audit with you</p>
        <div className={`rounded-2xl p-8 mb-8 text-center ${isLowSavings ? 'bg-blue-50' : 'bg-emerald-50'}`}>
          {isLowSavings ? (
            <div>
              <div className="text-4xl mb-3">✅</div>
              <h1 className="text-2xl font-bold text-gray-900 mb-2">This stack is spending well.</h1>
              <p className="text-gray-500">Their AI stack looks optimised for their team size and use case.</p>
            </div>
          ) : (
            <div>
              <p className="text-emerald-600 font-medium text-sm uppercase tracking-wide mb-2">Potential Savings Found</p>
              <h1 className="text-5xl font-bold text-gray-900 mb-1">${data.total_monthly_savings.toFixed(0)}<span className="text-2xl text-gray-400">/mo</span></h1>
              <p className="text-emerald-600 font-semibold text-xl">${data.total_annual_savings.toFixed(0)} saved per year</p>
            </div>
          )}
        </div>
        {isHighSavings && (
          <div className="bg-gray-900 text-white rounded-2xl p-6 mb-8">
            ...
            <p className="text-gray-400 text-sm mb-4">Credex sources discounted AI credits from companies that overforecast. Save an additional 20-40%.</p>
        ...
        <div className="bg-white rounded-2xl p-6 shadow-sm text-center">
          <h3 className="font-semibold text-gray-900 mb-2">Audit your own AI spend</h3>
          <p className="text-sm text-gray-500 mb-4">Free, no login required. Takes 2 minutes.</p>
          <a href="/audit" className="inline-block bg-emerald-600 text-white px-6 py-3 rounded-full font-semibold text-sm hover:bg-emerald-700 transition">Start My Free Audit</a>
        </div>
      </div>
    </main>
  )
